...
--   exact (UniformOnFun.uniform_continuous_eval_of_mem _ _ e1 e2).continuous.comp_ContinuousOn
--     (ContinuousOn_uderiv hU),
-- end

-- theorem main [good_domain U] : ∃ f ∈ 𝓘 U, f '' U = ball (0 : ℂ) 1 :=
-- begin
--   obtain ⟨z₀, hz₀⟩ : U.nonempty := good_domain.is_nonempty,
--   have hU : IsOpen U := good_domain.IsOpen,
--   have hU' : is_preconnected U := good_domain.is_preconnected,
...
--     contrapose! this,
--     obtain ⟨w, hw : eq_on f (λ z, w) U⟩ := this,
--     simpa only [deriv_const'] using (hw.eventually_eq_of_mem (hU.mem_nhds hz₀)).deriv_eq },
--   have h5 : f ∈ 𝓘 U := ⟨hf.1, hf.2.resolve_right h7⟩,
--   refine ⟨f, h5, _⟩,
--   have h10 : f '' U ⊆ ball 0 1,
--   { have := ((hf.1.1.analytic_on hU).is_constant_or_IsOpen hU').resolve_left h7 U subset_rfl hU,
...
--   exact ⟨g, 𝓘_subset_𝓙 ⟨⟨g.is_diff, g.MapsTo.mono_right ball_subset_closedBall⟩, g.is_inj⟩, hg⟩
-- end
